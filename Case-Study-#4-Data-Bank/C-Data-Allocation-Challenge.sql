...
date_series as (
 select customer_id,
  ...
  from (
    select customer_id,
    max(txn_date) as last_date, min(txn_date) as first_date
from txn_deposit
    group by 1
    ) min_max_series
  ),
  ...
  count(txn_group) over (partition by ds.customer_id order by date_series) as txn_count
    ...
    customer_data as (
     select customer_id,
    date_series, case when txn_row_no < 30 then NULL
    when avg_last_30_days < 0 then 0
    else avg_last_30_days end as data_store
    from (
    select *,
    avg(txn_sum) over(partition by date_series rows between 30 preceding and current row) as avg_last_30_days,
    ...
    order by 1
      ...
      select
      month, round(sum(data_allocation),1) as total_allocation from (
        select customer_id,
