...
SELECT
    (SELECT
        COUNT(DISTINCT interest_id) AS num_of_metric_id
    FROM fresh_segments.interest_metrics
...
--6. What sort of table join should we perform for our analysis and why? Check
-- your logic by checking the rows where interest_id = 21246 in your joined
-- output and include all columns from fresh_segments.interest_metrics and all columns from fresh_segments.interest_map except from the id column.
SELECT
    ime.*,
    interest_name,
    interest_summary,
...
-- before the created_at value from the fresh_segments.interest_map table? Do you think these values are valid and why?
